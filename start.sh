...
# 2. Динамическая установка Ollama внутри пода (если её еще нет)
if [ ! -f "/usr/bin/ollama" ]; then
    echo "Installing Ollama directly inside RunPod..."
    curl -L https://ollama.com/download/ollama-linux-amd64.tgz -o ollama-linux-amd64.tgz
    tar -C /usr -xzf ollama-linux-amd64.tgz
    rm ollama-linux-amd64.tgz
fi

# 3. Запуск Ollama в фоне
export OLLAMA_HOST=0.0.0.0:11434
ollama serve &
sleep 5

# 4. Проверка и скачивание моделей (выполняется только при первом запуске)
if [ ! -f "/root/.ollama/models/manifests/registry.ollama.ai/library/gemma4" ]; then
    echo "First run: initialising heavy LLM models..."
